...
### Fixed
- Fixed the `lightspeed/dot` icon (`assets/icons/lightspeed/dot.svg`) rendering too small: its circle only had a `r="28"` radius in a `256×256` viewBox (~22% coverage), leaving excessive transparent padding. The circle now fills the viewBox (`cx="128" cy="128" r="128"`), matching the intended eyebrow-bullet proportion used across the footer, homepage, blog, work, 404, and Services page. (LS-4168)
